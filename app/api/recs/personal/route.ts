...
async function discover(kind:"movie"|"tv", page:number, language:string, region:string, certMax?:string) {
  const base = new URLSearchParams({
    include_adult:"false", language, region, sort_by:"popularity.desc", page:String(page)
  });
  const url = certMax
...
async function providers(kind:"movie"|"tv", id:number, region:string): Promise<ProvidersSE> {
  const r = await fetch(`${TMDB}/${kind}/${id}/watch/providers`, { headers: H, next: { revalidate: 600 } });
  if (!r.ok) return null;
...
export async function GET(req: Request) {
  try {
    ...
    const allowed = new Set((Array.isArray(profile.providers) ? (profile.providers as unknown as string[]) : [])
    ...
    for (let i=0;i<subset.length && recs.length<limit;i+=8) {
      ...
      for (let j=0;j<batch.length && recs.length<limit;j++) {
        const it = batch[j]; const se = provs[j];
        const names = (se?.flatrate ?? []).map(p => p.provider_name);
        const norm = names.map(n => n.toLowerCase().replace(/\s+/g," ").replace(/\+/g,"plus").trim());
        const unknown = !se || !se.flatrate;
        const has = allowed.size > 0 ? norm.some(n => allowed.has(n)) : norm.length > 0;
        if (has) recs.push({ type:"rec", tmdbId: it.id, mediaType: it.mediaType, title: it.title, matchedProviders: names, unknown });
      }
    }
    ...
    if (hasLifetime) {
      ...
      recs.forEach((r, idx) => {
        if (idx>0 && idx%adEvery===0) {
          feed.push({ type:"ad", id:`house-${idx/adEvery}`, headline:"Uppgradera till Premium", body:"Större grupper, inga annonser.", cta:"Köp lifetime", href:"/premium" });
        }
        feed.push(r);
      });
    }
